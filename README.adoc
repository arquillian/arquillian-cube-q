...
Some of the discrete values set in some toxic like `slowClose`, `bandwidth`, `timeout` or `slice` can be randomized using mathematical distributions.
At this time two distributions are supported:

* Uniform Distribution: Distribution that returns values uniformally distributed across a range.
* LogNormal Distribution: Returns log normally distributed values. You can use this website https://www.wolframalpha.com/input/?i=lognormaldistribution%28log%2890%29%2C+0.1%29 to play with values

An as example of randomized latency:
...
In previous configuration latency times are distributed in using a log normal distribution with median of 2 seconds and 0.3 as sigma value.
Then for each iteration of the test, a new value is calculated and sent to toxiproxy.
